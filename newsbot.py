...
async def scrape_rss(client: httpx.AsyncClient, url: str, source_name: str, lookback_minutes: int | None = None):
    ...
    for e in feed.entries:
        ...
        if not is_today_et(dt):
            continue
        items.append({"source": source_name, "title": title, "url": link, "published_at": to_iso(dt), "raw_text": ""})
    return items
...
async def scrape_html(client: httpx.AsyncClient, cfg: dict, source_name: str):
    ...
    for node in soup.select(sel_item):
        ...
        if title:
            out.append({"source": source_name, "title": title, "url": link, "published_at": to_iso(dt), "raw_text": ""})
    return out

async def scrape_playwright(cfg: dict, source_name: str):
    # Not used (disabled), but kept for completeness
    ...
    async with async_playwright() as p:
        ...
        try: await page.wait_for_selector(wait_selector, timeout=15000)
        except Exception: pass
        for _ in range(3):
            await page.mouse.wheel(0, 2000); await page.wait_for_timeout(800)
        ...
        for it in items[:150]:
            ...
            if link: link = httpx.URL(link, base=url).human_repr()
            time_el = await it.query_selector(time_selector)
            tstr = (await time_el.inner_text()).strip() if time_el else ""
            dt = safe_parse_date(tstr) or now_et()
            if not is_today_et(dt): continue
            if title:
                out.append({"source": source_name, "title": normalize_whitespace(title), "url": link or url, "published_at": to_iso(dt), "raw_text": ""})
        await browser.close()
    return out
...
def filter_items(items: list[dict], cfg: dict, override_keywords=None, override_tickers=None) -> list[dict]:
    ...
    if not kw_list and not tk_list:
        return items
    out = []
    kw_pat = _re.compile("|".join([_re.escape(k) for k in kw_list]), _re.IGNORECASE) if kw_list else None
    for it in items:
...
def enrich_and_summarize(items: list[dict], max_sentences: int = 2) -> list[dict]:
    # title-only downstream; keep summary empty
...
async def run_once(cfg_path: str, send: bool = False, out_jsonl: str | None = "news.jsonl", out_csv: str | None = "news.csv", debug: bool = False, kw_override=None, tk_override=None):
        ...
        for s in sources:
            ...
            try:
                if typ == "rss":
                    ...
                    print(f"[WARN] Unknown source type: {typ}", file=sys.stderr); items = []
    ...
    if out_csv:
        ...
        with open(out_csv, "a", encoding="utf-8", newline="") as f:
            ...
            for it in all_items:
                w.writerow({"fetched_at": ts, "source": it.get("source"), "published_at": it.get("published_at"), "title": it.get("title"), "summary": it.get("summary"), "url": it.get("url")})

async def run_interval(cfg_path: str, seconds: int, send: bool, kw_override=None, tk_override=None):
    while True:
        try: await run_once(cfg_path, send, kw_override=kw_override, tk_override=tk_override)
...
def main():
    ...
    if args.once or args.interval <= 0:
        ...
        asyncio.run(run_interval(args.config, args.interval, send=args.send, kw_override=kw_ov if kw_ov else None, tk_override=tk_ov if tk_ov else None))
